...
export const CitizenServiceUpdate = () => {
  ...
  const [isElectronicValue, setIsElectronicValue] = useState<boolean>(newEntityDefaults.current.isElectronic);
  ...
  useEffect(() => {
    if (isNew) {
      setIsElectronicValue(newEntityDefaults.current.isElectronic);
    } else {
      setIsElectronicValue(!!citizenServiceEntity?.isElectronic);
    }
  }, [citizenServiceEntity?.isElectronic, isNew]);
  ...
  const saveEntity = values => {
    ...
    const sanitizedServiceLink = values.isElectronic ? normalizeValue(values.serviceLink) : undefined;

    const entity = {
      ...citizenServiceEntity,
      ...values,
      serviceLink: sanitizedServiceLink,
  ...
  const defaultValues = useMemo(() => {
    ...
    return {
      estimatedDurationUnit: 'MINUTE',
      ...citizenServiceEntity,
      serviceLink: citizenServiceEntity?.serviceLink,
          ...
          {loading ? (
            ...
            <ValidatedForm defaultValues={defaultValues} onSubmit={saveEntity}>
              ...
              <ValidatedField
                ...
                check
                type="checkbox"
                onChange={event => setIsElectronicValue(event.target.checked)}
              />
              {isElectronicValue ? (
                <ValidatedField
                  label={translate('citizenServicesApp.citizenService.serviceLink')}
                  id="citizen-service-serviceLink"
                  name="serviceLink"
                  data-cy="serviceLink"
                  type="text"
