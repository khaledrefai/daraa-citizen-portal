...
public class AiAssistantService {
    ...
    public void indexCitizenService(CitizenServiceDTO citizenServiceDTO) {
        ...
        LOG.info("AI assistant indexing started for citizen service {} (id={})", citizenServiceDTO.getName(), citizenServiceDTO.getId());

        try {
            prepareCollection();
            String document = buildServiceDocument(citizenServiceDTO);
            LOG.info("Generating embedding for citizen service {}", citizenServiceDTO.getId());
            long embedStart = System.currentTimeMillis();
            List<Double> embedding = embedText(document);
            LOG.info("Embedding ready for citizen service {} ({} ms)", citizenServiceDTO.getId(), System.currentTimeMillis() - embedStart);
            Map<String, Object> payload = buildPayload(citizenServiceDTO);
            LOG.info("Saving vector to Qdrant for citizen service {}", citizenServiceDTO.getId());
            long upsertStart = System.currentTimeMillis();
            upsertPoint(citizenServiceDTO.getId(), embedding, payload);
            LOG.info(
                "Qdrant upsert completed for citizen service {} ({} ms)",
                citizenServiceDTO.getId(),
                System.currentTimeMillis() - upsertStart
            );
            LOG.info(
                "AI assistant indexing finished for citizen service {} ({} ms total)",
                citizenServiceDTO.getId(),
